...
static void write_cmd(uint8_t cmd)
{
    ...
    spi_bsp_transfer(_spi_dev_id, &cmd, 1);
}

static void write_data(const uint8_t *data, size_t len)
{
    gpio_set_level(_cfg.dc_pin, 1);
    spi_bsp_transfer(_spi_dev_id, data, len);
}
...
esp_err_t st7789_init(const st7789_config_t *cfg)
{
    ...
    esp_err_t ret = spi_bsp_add_device(&_spi_dev_id, _cfg.spi_host, _cfg.cs_pin, _cfg.clk_speed_hz, 3, "ST7789");
    if (ret != ESP_OK) return ret;

    if (_cfg.bl_pin != GPIO_NUM_NC) {
        pwm_bsp_init(_cfg.bl_pin, 5000, LEDC_TIMER_8_BIT, LEDC_LOW_SPEED_MODE, LEDC_TIMER_1, LEDC_CHANNEL_1);
        pwm_bsp_set_duty(LEDC_LOW_SPEED_MODE, LEDC_CHANNEL_1, 0);
    }
    ...
    if (_cfg.bl_pin != GPIO_NUM_NC) {
        pwm_bsp_set_duty(LEDC_LOW_SPEED_MODE, LEDC_CHANNEL_1, 80);
    }
...
esp_err_t st7789_set_backlight(uint8_t brightness_percent)
{
    ...
    return pwm_bsp_set_duty(LEDC_LOW_SPEED_MODE, LEDC_CHANNEL_1, brightness_percent);
}
...
{
    gpio_set_level(_cfg.dc_pin, 1);
    return spi_bsp_transfer(_spi_dev_id, (const uint8_t *)colors, len * 2);
}

esp_err_t st7789_push_colors_dma(const uint16_t *colors, uint32_t len)
{
    gpio_set_level(_cfg.dc_pin, 1);
    return spi_bsp_queue_transfer(_spi_dev_id, (const uint8_t *)colors, len * 2);
}
