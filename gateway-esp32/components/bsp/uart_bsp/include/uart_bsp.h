/**
 * @file uart_bsp.h
 * @brief Thread-safe Generic UART BSP for ESP32
 * 
...
#define UART_BSP_DEFAULT_PORT      UART_NUM_2
#define UART_BSP_DEFAULT_BAUD      115200
#define UART_BSP_RX_BUF_SIZE       (2048)
#define UART_BSP_TX_BUF_SIZE       (1024)
 ...
 */
typedef void (*uart_bsp_data_cb_t)(const uint8_t* data, size_t len);

/**
 * @brief Configuration structure for UART BSP
 */
typedef struct {
    uart_port_t port;
    int baud_rate;
    int tx_io_num;
    int rx_io_num;
    uart_bsp_data_cb_t data_cb; // Optional data callback
} uart_bsp_config_t;
...
/**
 * @brief Deinitialize the UART BSP
 * 
 * @return esp_err_t ESP_OK on success
 */
 ...
 * 
 * @return uart_port_t The UART port number used by this BSP
 */
