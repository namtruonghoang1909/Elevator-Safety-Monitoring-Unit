...
esp_err_t spi_bsp_queue_transfer(uint8_t dev_id, const uint8_t *data, size_t len)
{
    ...
    // For simplicity in this BSP, we assume the caller manages the buffer lifetime.
    // In a professional driver, we'd use a pool of transaction descriptors.
    static spi_transaction_t trans; // WARNING: This only supports ONE inflight DMA transfer per system in this simplified BSP
