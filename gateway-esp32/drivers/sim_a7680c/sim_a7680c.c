...
static esp_err_t wait_for_terminal(const char* expected_prefix, char* out_line, size_t out_len, uint32_t timeout_ms) {
    ...
    while ((xTaskGetTickCount() - start_tick) < pdMS_TO_TICKS(timeout_ms)) {
        int len = platform_uart_read_line(line, sizeof(line), 500);
        if (len <= 0) continue;
...
esp_err_t sim_a7680c_init(void) {
    ...
    platform_uart_set_data_cb(NULL);
    platform_uart_flush();
    ...
    platform_uart_send_str("ATE0\r\n");      // Echo Off
    wait_for_terminal(NULL, NULL, 0, 1000);
    platform_uart_send_str("AT+CMEE=2\r\n");  // Verbose Errors
    wait_for_terminal(NULL, NULL, 0, 1000);
    platform_uart_send_str("AT+CNMP=2\r\n");  // Preferred mode: Automatic (LTE/GSM)
    wait_for_terminal(NULL, NULL, 0, 1000);
    platform_uart_send_str("AT+CFUN=1\r\n");  // Radio Full
...
esp_err_t sim_a7680c_get_sim_info(sim_a7680c_info_t* info) {
    ...
    // Get IMSI
    platform_uart_send_str("AT+CIMI\r\n");
    if (wait_for_terminal("", line, sizeof(line), 1000) == ESP_OK) {
    ...
    platform_uart_send_str("AT+CNUM\r\n");
    ...
    // Get Operator
    platform_uart_send_str("AT+COPS?\r\n");
    ...
    bool reg = false;
    platform_uart_send_str("AT+CEREG?\r\n"); // Check LTE first
    ...
    if (!reg) {
        platform_uart_send_str("AT+CREG?\r\n");
...
esp_err_t sim_a7680c_check_status(void) {
    ...
    // 1. PIN Status
    platform_uart_send_str("AT+CPIN?\r\n");
    ...
    // 2. RF Functionality
    platform_uart_send_str("AT+CFUN?\r\n");
...
esp_err_t sim_a7680c_send_sms(const char* phone, const char* message) {
    ...
    platform_uart_send_str("AT+CMGF=1\r\n");
    wait_for_terminal(NULL, NULL, 0, 1000);
    ...
    for(int i=0; i<20; i++) {
        if(platform_uart_read_line(&c, 1, 100) > 0 && c == '>') { got_prompt=true; break; }
    }
    
    esp_err_t err = ESP_FAIL;
    if (got_prompt) {
        platform_uart_send_str(message);
        const uint8_t ctrl_z = 0x1A; platform_uart_write(&ctrl_z, 1);
