/**
 * @file uart_platform.c
 * @brief Implementation of the generic thread-safe UART Platform Layer.
 */
...
static const char *TAG = "uart_platform";

// ─────────────────────────────────────────────
// Private variables, macros
// ─────────────────────────────────────────────

static uart_port_t g_uart_port = UART_PLATFORM_DEFAULT_PORT;
static SemaphoreHandle_t g_tx_mutex = NULL;
static SemaphoreHandle_t g_rx_mutex = NULL;
static platform_uart_data_cb_t g_data_callback = NULL;
 ...
 */
static void platform_uart_task(void *pvParameters) {
    uint8_t data[256];
...
esp_err_t platform_uart_init(const platform_uart_config_t* config) {
    ...
    ESP_ERROR_CHECK(uart_driver_install(g_uart_port, UART_PLATFORM_RX_BUF_SIZE, UART_PLATFORM_TX_BUF_SIZE, 0, NULL, 0));
    ...
    if (g_data_callback) {
        xTaskCreate(platform_uart_task, "platform_uart_task", 4096, NULL, 10, &g_uart_task_handle);
    }

    g_is_initialized = true;
    ESP_LOGI(TAG, "Generic UART Platform Layer initialized on port %d", g_uart_port);
...
esp_err_t platform_uart_send_str(const char* str) {
    int len = strlen(str);
    if (platform_uart_write((const uint8_t*)str, len) == len) {
        return ESP_OK;
    }
    return ESP_FAIL;
}
