/**
 * @file i2c_platform.c
 * @brief Thread-safe I2C Platform Abstraction implementation for STM32
 */
...
static platform_i2c_status_t map_hal_status(HAL_StatusTypeDef status) {
    switch (status) {
        case HAL_OK:      return PLATFORM_I2C_OK;
        case HAL_BUSY:    return PLATFORM_I2C_BUSY;
        case HAL_TIMEOUT: return PLATFORM_I2C_TIMEOUT;
        case HAL_ERROR:
        default:          return PLATFORM_I2C_ERROR;
    }
}
...
platform_i2c_status_t platform_i2c_init(void) {
    ...
    return (i2c_mutex != NULL) ? PLATFORM_I2C_OK : PLATFORM_I2C_ERROR;
}
...
platform_i2c_status_t platform_i2c_write_reg(uint8_t dev_addr, uint8_t reg, uint8_t val) {
    if (i2c_mutex == NULL) return PLATFORM_I2C_ERROR;

    platform_i2c_status_t status = PLATFORM_I2C_ERROR;
